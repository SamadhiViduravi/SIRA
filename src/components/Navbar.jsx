import { Link } from 'react-router-dom';

export function Navbar() {
  return (
    <nav className="bg-crimson text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          
          {/* Left Side: Brand and Internal Links */}
          <div className="flex items-center space-x-8">
            <Link to="/" className="text-xl font-bold hover:text-gray-200 transition">
              SIRA
            </Link>
            <div className="hidden md:flex space-x-6">
              <Link
                to="/"
                className="px-3 py-2 rounded-md text-sm font-medium hover:bg-crimson-dark transition"
              >
                GPA Calculator
              </Link>
              <Link
                to="/programs"
                className="px-3 py-2 rounded-md text-sm font-medium hover:bg-crimson-dark transition"
              >
                Programs
              </Link>
              <Link
                to="/resources"
                className="px-3 py-2 rounded-md text-sm font-medium hover:bg-crimson-dark transition"
              >
                Resources
              </Link>
              <Link
                to="/articles"
                className="px-3 py-2 rounded-md text-sm font-medium hover:bg-crimson-dark transition"
              >
                Articles Hub
              </Link>
            </div>
          </div>

          {/* Right Side: GitHub Contribution Call-to-Action Button */}
          <div className="flex items-center">
            <a
              href="https://github.com/SamadhiViduravi/SIRA"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center space-x-2 bg-white text-crimson px-4 py-2 rounded-md text-sm font-semibold hover:bg-gray-100 hover:text-crimson-dark transition shadow-md"
            >
              {/* Inline SVG GitHub Icon */}
              <svg
                className="w-4 h-4 fill-current"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.48 text-nonev-1.674c-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852v2.747c0 .267.18.577.688.479C19.138 20.164 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
              </svg>
              <span>Contribute</span>
            </a>
          </div>

        </div>
      </div>
    </nav>
  );
}